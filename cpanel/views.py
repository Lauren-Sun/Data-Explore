...
def postsignup(request):
    ...
    return render(request,'signIn.html')
def getdata(request):
    dataset=request.GET.get('Database')
    ...
    for i in keyword:
        ...
        if value is not None:
            ...
            return render(request,'welcome.html',{"info":"Not found, please search other words."})
    ...
    def takeSecond(elem):
        return elem[1]
    fin_res.sort(key=takeSecond,reverse=True)
    ranked_result=[]
    for i in fin_res:
        ranked_result.append(i[0])
    # # for i in fin_dic:
    #     ranked_result.append(i[1])

    return render(request,'welcome.html',{'data_list':json.dumps(ranked_result),'dataset':json.dumps(dataset),'key':json.dumps(keyword),'result':json.dumps(fin_res)})
def selectdata(request):
    pri_key = request.GET['name']
    table = request.GET['table']
    link = request.GET['sub']
    dataset=request.GET['dataset']
    value = database.child(table).child(link).child(pri_key).get().val()
    return render(request,'welcome.html',{'data':json.dumps(value),'dataset':json.dumps(dataset)})
